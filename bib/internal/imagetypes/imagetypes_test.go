...
func TestImageTypes(t *testing.T) {
	testCases := map[string]testCase{
		...
		"bad-image-type": {
			...
			expectedErr: errors.New(`unsupported image type "bad", valid types are ami, anaconda-iso, bootc-installer, gce, iso, ova, pxe-tree, qcow2, raw, vhd, vmdk`),
		},
		"bad-in-good": {
			imageTypes:  []string{"ami", "raw", "vmdk", "qcow2", "something-else-what-is-this"},
			expectedErr: errors.New(`unsupported image type "something-else-what-is-this", valid types are ami, anaconda-iso, bootc-installer, gce, iso, ova, pxe-tree, qcow2, raw, vhd, vmdk`),
		},
		"all-bad": {
			imageTypes:  []string{"bad1", "bad2", "bad3", "bad4", "bad5", "bad42"},
			expectedErr: errors.New(`unsupported image type "bad1", valid types are ami, anaconda-iso, bootc-installer, gce, iso, ova, pxe-tree, qcow2, raw, vhd, vmdk`),
		},
	}
